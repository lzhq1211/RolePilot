...
function renderHeader(state) {
  ...
  if (contactEl) {
    ...
    const items = [
      ...
      ...["location", "website", "github"].map((field) => profile[field] && { field, text: profile[field] }),
      ...(profile.extraContacts || []).map((contact, index) => ({
        field: `extraContacts.${index}.value`, text: contact.value, label: contact.label,
      })),
    ].filter(Boolean);
...
function updateAddGutter(state) {
  ...
  state.sections.forEach((section) => {
    const sectionEl = document.querySelector(`section[data-section-id="${CSS.escape(section.id)}"]`);
    if (!sectionEl) return;
    ...
    section.entries.forEach((entry) => {
      const bulletsEl = sectionEl.querySelector(`ul[data-entry-id="${CSS.escape(entry.id)}"]`);
      if (!bulletsEl) return;
