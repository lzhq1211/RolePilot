...
function markDirty() {
  ...
  if (typeof notifyWorkbenchChange === "function") notifyWorkbenchChange();
}
...
function initSelectionFormatting() {
  ...
  const shell = document.querySelector(".selection-format-shell");
  const capsule = shell && shell.querySelector(".selection-format-capsule");
  const moveCapsule = (button) => {
    if (!shell || !capsule || !button || button.disabled) return;
    capsule.style.setProperty("--selection-cap-left", `${button.offsetLeft}px`);
    capsule.style.setProperty("--selection-cap-width", `${button.offsetWidth}px`);
  };
  Object.values(buttons).forEach((button) => {
    if (!button) return;
    button.addEventListener("mouseenter", () => moveCapsule(button));
...
/** Clear transient history and selection state when switching documents. */
...
function getBlockFormatSelector(key) {
  ...
  const field = kind === "profile" ? undefined : parts.pop();
  const id = parts.join(":");
  return kind === "profile"
  ...
  for (const section of state.sections) {
    for (const entry of getSectionBulletContainers(section)) {
      ...
      const listEl = document.querySelector(
        `[data-entry-id="${CSS.escape(entry.id)}"].entry-bullets, [data-entry-id="${CSS.escape(entry.id)}"].skills-list`
      );
      if (listEl) {
        listEl.innerHTML = "";
        for (const b of entry.bullets) listEl.appendChild(renderBulletRow(b));
        // Focus new bullet
        const newSpan = listEl.querySelector(`[data-bullet-id="${CSS.escape(newBullet.id)}"]`);
...
function addBullet(entryId) {
  ...
  for (const section of state.sections) {
    for (const entry of getSectionBulletContainers(section)) {
      ...
      const listEl = document.querySelector(
        `ul[data-entry-id="${CSS.escape(entryId)}"]`
      );
      if (listEl) {
        ...
        const newSpan = newLi.querySelector(`[data-bullet-id="${CSS.escape(newBullet.id)}"]`);
        focusWithoutUndoSnapshot(newSpan);
      }
...
function deleteBullet(bulletId) {
  ...
  for (const section of state.sections) {
    for (const entry of getSectionBulletContainers(section)) {
      ...
      const li = document.querySelector(`li[data-bullet-id="${CSS.escape(bulletId)}"]`);
      if (li) li.remove();
      markDirty();
      requestAnimationFrame(() => updateAddGutter(state));
      return;
...
function addEntry(sectionId) {
  ...
  if (sectionEl) {
    ...
    // Focus name field
    const nameSpan = sectionEl.querySelector(`[data-entry-id="${CSS.escape(newEntry.id)}"] .entry-name`);
      ...
      buttons: [
        { text: "取消" },
        {
          ...
          action: () => {
            ...
            const el = document.querySelector(`[data-entry-id="${CSS.escape(entryId)}"].resume-entry`);
  ...
  if (el.dataset.profileField) {
    ...
    if (field === "headline") {
      ...
      state.profile.portfolio = raw && current && current.href
        ? `${raw} | ${current.href}`
        : raw;
    } else if (/^extraContacts\.\d+\.value$/.test(field)) {
      const contact = state.profile.extraContacts?.[Number(field.split(".")[1])];
      if (contact) contact.value = raw;
...
function tokensFromEditableElement(element) {
  ...
  const walk = (node, strong = false, italic = false, inheritedDelta = 0, inheritedHref = "") => {
    ...
    if (node.tagName === "BR") {
      tokens.push({ type: strong ? "strong" : "text", value: "\n", ...(italic ? { italic: true } : {}) });
      return;
    }
    if (["DIV", "P"].includes(node.tagName) && tokens.length && !tokens[tokens.length - 1].value.endsWith("\n")) {
...
function initSpacingHandles() {
  ...
  container.addEventListener("pointermove", (e) => {
    ...
    const sectionEl = container.querySelector(`section[data-section-id="${CSS.escape(activeSectionId)}"]`);
