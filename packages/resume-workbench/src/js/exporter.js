...
function serializeStateToMarkdown(state) {
  ...
  if (state.profile.extraContacts?.length) {
    lines.push(`extra_contacts: ${JSON.stringify(state.profile.extraContacts)}`);
  }
  lines.push("---", "");
  ...
  (state.sections || []).forEach((section) => {
      ...
      (section.blocks || []).forEach((block) => {
        ...
        if (block.type === "list") {
          (block.bullets || []).forEach((bullet) => {
            lines.push(`- ${serializeInlineMarkdown(bullet.content).replace(/\n/g, "\n  ")}`);
          });
          lines.push("");
          return;
        }
    ...
    (section.entries || []).forEach((entry) => {
      if (section.type === "skills" && section.entries.length === 1 && !entry.name && !entry.role && !entry.date && !entry.location) {
        (entry.bullets || []).forEach((bullet) => lines.push(`- ${serializeInlineMarkdown(bullet.content).replace(/\n/g, "\n  ")}`));
        lines.push("");
...
function serializeInlineMarkdown(tokens) {
  const semanticTokens = (tokens || []).reduce((result, token) => {
    const previous = result[result.length - 1];
    if (previous && previous.type === token.type && Boolean(previous.italic) === Boolean(token.italic) && previous.href === token.href) {
      previous.value += token.value || "";
    } else result.push({ ...token });
    return result;
  }, []);
  return semanticTokens.map((token) => {
    const value = String(token.value || "").replace(/[\\*\[\]]/g, "\\$&");
...
function serializeStateToJson(state) {
  return JSON.stringify(stateToResumeContent(state), null, 2) + "\n";
}
...
function exportPdf() {
  ...
  clone.classList.add("print-export-copy");
  clone.setAttribute("aria-hidden", "true");
  document.body.appendChild(clone);
  app.classList.add("print-export-source-hidden");
  const cleanup = () => {
    clone.remove();
    app.classList.remove("print-export-source-hidden");
